...
#[cfg(test)]
mod tests {
    use proof_gen::{ConsolidationProofBundle, GindexCalculator};
    ...
    fn load_test_vectors() -> TestVectors {
        ...
        let content = fs::read_to_string(&vectors_path).unwrap_or_else(|e| {
            panic!("Failed to read test vectors from {:?}: {}", vectors_path, e)
        });
    ...
    fn test_proof_lengths_match_gindex_depth() {
        ...
        assert_eq!(validator_depth, GindexCalculator::validator_proof_length());
    }
    ...
    fn test_proof_bundle_serialization() {
        ...
        assert_eq!(deserialized.consolidation_index, bundle.consolidation_index);
        assert_eq!(deserialized.source_index, bundle.source_index);
        assert_eq!(deserialized.activation_epoch, bundle.activation_epoch);
        assert_eq!(deserialized.source_credentials, bundle.source_credentials);
        assert_eq!(
    ...
    fn test_cross_validate_test_vectors_structure() {
        ...
        for (i, claim) in vectors.claims.iter().enumerate() {
            ...
            let expected_consolidation_len =
                GindexCalculator::consolidation_proof_length() as usize;
            let expected_validator_len = GindexCalculator::validator_proof_length() as usize;
    ...
    fn test_invalid_claims_variety() {
        ...
        assert!(has_tampered_proof, "Should have tampered proof test cases");
    ...
    fn test_gindex_increasing_for_sequential_indices() {
        ...
        assert_eq!(
            gindex1,
            gindex0 + 2,
            "Consolidation gindices should differ by 2"
        );
        assert_eq!(
            gindex2,
            gindex1 + 2,
        ...
        assert_eq!(
            val_gindex1,
            val_gindex0 + 8,
            "Validator gindices should differ by 8"
        );
    }
}
