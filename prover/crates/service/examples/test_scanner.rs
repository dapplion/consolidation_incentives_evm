...
async fn main() -> anyhow::Result<()> {
    let beacon_url = std::env::var("BEACON_API_URL")
        .unwrap_or_else(|_| "http://localhost:15052".to_string());
    ...
    println!("🔍 Fetching pending consolidations at slot {}...", finalized_slot);
    let consolidations = client.get_pending_consolidations(&finalized_slot.to_string()).await?;
    ...
    if !consolidations.is_empty() {
        ...
        for (i, c) in consolidations.iter().enumerate() {
            println!("   {}. source: {} → target: {}", i+1, c.source_index, c.target_index);
        }
        println!();
    }
